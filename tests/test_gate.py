...
class TestConfirmMode:
    ...
    def test_a_print_waits_for_the_operator(self, confirm_pipeline, make_invoice):
        job = run(confirm_pipeline, make_invoice)
        assert job.status is JobStatus.AWAITING

    def test_it_waits_even_when_a_number_is_on_the_page(self, confirm_pipeline, make_invoice):
    ...
    def test_it_waits_when_there_is_no_number_at_all(self, confirm_pipeline, make_invoice):
    ...
    def test_the_message_is_ready_for_the_dialog_to_show(self, confirm_pipeline, make_invoice):
        job = run(confirm_pipeline, make_invoice)
        assert "INV-2291" in job.message_preview
        assert "18,450.00" in job.message_preview

    def test_the_operator_sends_it(self, confirm_pipeline, make_invoice):
        job = run(confirm_pipeline, make_invoice, InvoiceSpec(customer_phone=None))
        sent = confirm_pipeline.release(job.id, "98765 43210")
        assert sent.status is JobStatus.DRY_RUN
        assert sent.recipient == "+919876543210"

    def test_the_operator_can_correct_the_name(self, confirm_pipeline, make_invoice):
        job = run(confirm_pipeline, make_invoice, InvoiceSpec(customer_phone=None))
        sent = confirm_pipeline.release(job.id, "9876543210", customer_name="Ravi Kumar")
        assert "Ravi Kumar" in sent.message_preview

    def test_skipping_leaves_it_in_the_queue(self, confirm_pipeline, make_invoice):
        job = run(confirm_pipeline, make_invoice)
        deferred = confirm_pipeline.defer(job.id)
        assert deferred.status is JobStatus.HELD
        assert "Skipped at the print dialog" in deferred.hold_reason

    def test_a_skipped_job_can_still_be_sent_later(self, confirm_pipeline, make_invoice):
        job = run(confirm_pipeline, make_invoice)
        confirm_pipeline.defer(job.id)
        sent = confirm_pipeline.release(job.id, "9876543210")
        assert sent.status is JobStatus.DRY_RUN

    def test_a_bad_number_is_refused(self, confirm_pipeline, make_invoice):
        import pytest

        job = run(confirm_pipeline, make_invoice)
        with pytest.raises(ValueError, match="not a valid mobile"):
            confirm_pipeline.release(job.id, "12345")

    def test_a_job_cannot_be_sent_twice(self, confirm_pipeline, make_invoice):
        import pytest

        job = run(confirm_pipeline, make_invoice)
        confirm_pipeline.release(job.id, "9876543210")
        with pytest.raises(ValueError, match="already sent"):
            confirm_pipeline.release(job.id, "9876543210")

    def test_an_unreadable_file_fails_without_killing_the_service(
        self, confirm_pipeline, tmp_path
    ):
        broken = tmp_path / "broken.pdf"
        broken.write_bytes(b"this is not a pdf")
        job = confirm_pipeline.process(broken)
        assert job.status is JobStatus.FAILED
        assert job.error


class TestAutomaticSends:
    def test_a_clean_invoice_is_sent_without_asking(self, pipeline, make_invoice):
        job = run(pipeline, make_invoice)
        assert job.status is JobStatus.DRY_RUN
        assert job.recipient == "+919876543210"

    def test_the_message_preview_is_recorded(self, pipeline, make_invoice):
        job = run(pipeline, make_invoice)
...
class TestAutomaticHolds:
    def test_no_number_on_the_page(self, pipeline, make_invoice):
    ...
    def test_only_the_sellers_own_number(self, pipeline, make_invoice):
        spec = InvoiceSpec(customer_phone=None, seller_phone="9845012345")
        job = run(pipeline, make_invoice, spec)
        assert job.status is JobStatus.HELD

    def test_two_equally_good_numbers(self, pipeline, make_invoice):
        spec = InvoiceSpec(extra_customer_lines=["Transporter Mobile: 9812345678"])
        job = run(pipeline, make_invoice, spec)
        assert job.status is JobStatus.HELD
        assert "equally likely" in job.hold_reason

    def test_a_merely_plausible_number_is_not_sent(self, pipeline, make_invoice):
        pipeline.settings.own_numbers = []
        job = run(pipeline, make_invoice, InvoiceSpec(customer_phone=None))
        assert job.status is JobStatus.HELD
...
class TestDeduplication:
    def test_a_reprint_does_not_send_twice(self, pipeline, make_invoice):
        first = run(pipeline, make_invoice)
        assert first.status is JobStatus.DRY_RUN

        second = run(pipeline, make_invoice)
    ...
    def test_a_different_invoice_to_the_same_customer_still_sends(
        ...
        run(pipeline, make_invoice)
        job = run(pipeline, make_invoice, InvoiceSpec(invoice_number="INV-2292"))
        assert job.status is JobStatus.DRY_RUN
    ...
    def test_the_same_invoice_sends_again_once_the_window_lapses(
        ...
        later = datetime.now() + timedelta(
            hours=pipeline.settings.dedupe_window_hours + 1
        )
        job = run(pipeline, make_invoice, now=later)
        assert job.status is JobStatus.DRY_RUN


class TestVolumeRails:
    def test_a_runaway_batch_is_capped(self, pipeline, make_invoice, monkeypatch):
        pipeline.settings.max_sends_per_minute = 3
        monkeypatch.setattr(pipeline.store, "count_sent_since", lambda _s: 3)
        job = run(pipeline, make_invoice)
        assert job.status is JobStatus.HELD
